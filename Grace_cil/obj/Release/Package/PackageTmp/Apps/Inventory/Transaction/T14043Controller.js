...
app.controller("T14043Controller", ["$scope", "Service", "Data", "$window", "$filter",
    function ($scope, Service, Data, $window, $filter) { //$location,
     ...
     //   loadData();
        ...
        $scope.obj.T14043.T_SALE_DATE = new Date();
        function generateMemoNo() {
            var date = $filter('date')(new Date(), 'dd-MM-yyyy');
            var product = Service.loadDataSingleParm('/T14043/GetMemoNo', date);
            product.then(function (returnData) {
                var memo = JSON.parse(returnData);
                $scope.obj.T14043.T_MEMO_NO = memo.T_MEMO_NO
                $scope.obj.T14043.T_MEMO_SQ = memo.T_MEMO_SQ
            });
        }
        $scope.onProductCode_Click = function () {
            var product = Service.loadDataSingleParm('/T14043/GetProductByCode', $scope.obj.T_PRODUCT_CODE);
            product.then(function (returnData) {
        ...
        function type() {
            var type = Service.loadDataWithoutParm('/T14043/GetTypeData');
        ...
        $scope.changeProduct = function (data) {
            var product = Service.loadDataSingleParm('/T14043/GetProductByCode', data.T_PRODUCT_CODE);
            product.then(function (returnData) {
                var pro = JSON.parse(returnData);
                if (pro.length > 0) {
        ...
        function allProduct() {
            var type = Service.loadDataWithoutParm('/T14043/GetAllProduct');
            type.then(function (returnData) {
        ...
        function discountCalculate() {
            vatTexCalculate();
            var dct = $scope.obj.T14043.T_DISCOUNT == undefined ? '0' : $scope.obj.T14043.T_DISCOUNT == '' ? '0' : $scope.obj.T14043.T_DISCOUNT;
            //$scope.obj.T14043.T_BALANCE = parseFloat($scope.obj.GRAND_TOTAL_PRICE) - parseFloat(dct);
            $scope.obj.T14043.T_BALANCE = (parseFloat($scope.obj.GRAND_TOTAL_PRICE) + parseFloat($scope.TotalVat)) - parseFloat(dct);
            $scope.obj.PAY = $scope.obj.T14043.T_BALANCE.toFixed(2);
        }
        ...
        function vatTexCalculate() {
            var dct = $scope.obj.T14043.T_DISCOUNT == undefined ? '0' : $scope.obj.T14043.T_DISCOUNT == '' ? '0' : $scope.obj.T14043.T_DISCOUNT;
            var vatTex = $scope.obj.T14043.T_VAT_TAX == undefined ? '0' : $scope.obj.T14043.T_VAT_TAX == '' ? '0' : $scope.obj.T14043.T_VAT_TAX;
            var grandTotal = parseFloat($scope.obj.GRAND_TOTAL_PRICE);
            $scope.TotalVat = (parseFloat(vatTex) * grandTotal) / 100;
            $scope.obj.T14043.T_TOTAL_VAT = $scope.TotalVat
            $scope.obj.T14043.T_BALANCE = (parseFloat($scope.obj.GRAND_TOTAL_PRICE) + parseFloat($scope.TotalVat)) - parseFloat(dct);
            $scope.obj.PAY = $scope.obj.T14043.T_BALANCE.toFixed(2);
        }
        $scope.keyup_Cash = function (e, ind) {
            if (parseFloat($scope.obj.T14043.T_PAMENT) > parseFloat($scope.obj.PAY)) {
                $scope.obj.T14043.T_PAMENT = $scope.obj.T14043.T_PAMENT.slice(0, -1);
            } else {
                dueCalculate();
            }
        ...
        $scope.Cust_Mobile_Click = function () {
            var mobile = $scope.obj.T14043.T_CUSTOMER_MOBILE;
            var customer = $scope.obj.CustomerList.filter(x => x.T_CUSTOMER_MOBILE == mobile);
            $scope.obj.T14043.T_CUSTOMER_ID = customer[0].T_CUSTOMER_ID;
            $scope.obj.T14043.T_CUSTOMER_MOBILE = customer[0].T_CUSTOMER_MOBILE;           
            $scope.obj.T14043.T_CUSTOMER_NAME = customer[0].T_CUSTOMER_NAME;
            $scope.obj.T14043.T_CUSTOMER_ADDRESS = customer[0].T_CUSTOMER_ADDRESS;

        }
        function getCustomerList() {
            var customer = Service.loadDataWithoutParm('/T14043/GetCustomerDetails');
        ...
        $scope.setCustomerRow = function (ind, data) {
            ...
            $scope.obj.T14043.T_CUSTOMER_ID = data.T_CUSTOMER_ID;
            $scope.obj.T14043.T_CUSTOMER_MOBILE = data.T_CUSTOMER_MOBILE;           
            $scope.obj.T14043.T_CUSTOMER_NAME = data.T_CUSTOMER_NAME;
            $scope.obj.T14043.T_CUSTOMER_ADDRESS = data.T_CUSTOMER_ADDRESS;
        ...
        function dueCalculate() {
            ...
            var due = $scope.obj.T14043.T_BALANCE == undefined ? '0' : $scope.obj.T14043.T_BALANCE == '' ? '0' : $scope.obj.T14043.T_BALANCE;
            var cash = $scope.obj.T14043.T_PAMENT == undefined ? '0' : $scope.obj.T14043.T_PAMENT == '' ? '0' : $scope.obj.T14043.T_PAMENT;
            if (parseFloat(cash) > parseFloat(totalDue)) {
                $scope.obj.T14043.T_PAMENT = '';
            } else {
                $scope.obj.T14043.T_BALANCE = (parseFloat(totalDue) - parseFloat(cash));
            }
        }
        function salePriceCalculate(ind, data) {
            var totalQut = 0;
        ...
        $scope.Save_Click = function () {
           ...
           // if (isEmpty('txtCustomerName')) { return; };
            loader(true)
            $scope.obj.T14043.T_SALE_DATE = $filter('date')($scope.obj.T14043.T_SALE_DATE, 'dd-MM-yyyy');           
            $scope.obj.T14043.T_ENTRY_DATE = $filter('date')($scope.obj.T14043.T_SALE_DATE, 'dd-MM-yyyy');
            $scope.obj.T14043.T_GRAND_TOTAL = $scope.obj.GRAND_TOTAL_PRICE;
            $scope.obj.T14043.T_SALE_TOTAL = $scope.obj.TOTAL_QUT;
            $scope.obj.T14043.T_TOTAL_VAT = $scope.TotalVat;
            $scope.obj.T14043.T_AFTER_DISCOUNT = $scope.obj.PAY;
            var save = Service.saveData_Model_List('/T14043/SaveData', $scope.obj.T14043, $scope.obj.dataList);
        ...
        function getMemolist() {
            var date = $filter('date')($scope.obj.T14043.T_SALE_DATE, 'dd-MM-yyyy');
            var memo = Service.loadDataSingleParm('/T14043/GetMemoList', date);
        ...
        $scope.setMemoNoRow = function (ind, data) {
            $scope.obj.T14043.T_SALE_ID = data.T_SALE_ID;
            $scope.obj.T14043.T_MEMO_NO = data.T_MEMO_NO;
            $scope.obj.T14043.T_CUSTOMER_NAME = data.T_CUSTOMER_NAME;
            $scope.obj.T14043.T_CUSTOMER_MOBILE = data.T_CUSTOMER_MOBILE;
            $scope.obj.T14043.T_CUSTOMER_ADDRESS = data.T_CUSTOMER_ADDRESS;
           // $scope.obj.T14043.T_SALE_DATE = data.T_SALE_DATE;
            $scope.obj.TOTAL_QUT = data.T_SALE_TOTAL;
            $scope.obj.GRAND_TOTAL_PRICE = data.T_GRAND_TOTAL;
            $scope.obj.T14043.T_DISCOUNT = data.T_DISCOUNT;
            $scope.obj.PAY = data.T_AFTER_DISCOUNT;
            $scope.obj.T14043.T_PAMENT = data.T_PAMENT;
            $scope.obj.T14043.T_BALANCE = data.T_DUE;
            ...
            var mDetails = Service.loadDataSingleParm('/T14043/GetMemoDetails', $scope.obj.T14043.T_SALE_ID);
