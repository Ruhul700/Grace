...
{
    public class T14037Controller : Controller
    {
        // GET: T14037
        T14037DAL repository = new T14037DAL();
        [HttpPost]
        public ActionResult GetStockShortList(T14040Parm paramList)
        {
            try
            {
                var shopId = Session["site"].ToString();
                var data = repository.GetStockShortList(paramList.T_PRODUCT, paramList.T_PACK, shopId);
        ...
        public ActionResult StockShortListReport(string product, string pack)
        {
            try
            {
                
                   // var shopId = Session["site"].ToString();
                    var details = repository.StockShortListReport(product, pack, "2");
