...
{
    public class T14024DAL : CommonDAL
    {
        public DataTable GetPaymentVerifyData(string shopId)
        {
            DataTable sql = new DataTable();
            sql = Query($@"SELECT ROW_NUMBER() OVER(Order by T_BALANCE_ID)SL,T_BALANCE_ID, T14080.T_MEM_INV_NO, T14010.T_CUSTOMER_NAME,T14010.T_CUSTOMER_ID, T_PAYMENT, T_DUE, T14080.T_ENTRY_DATE, T11020.T_USER_NAME T_EMP_NAME FROM T14080 JOIN T14010 ON T14080.T_CUSTOMER_ID = T14010.T_CUSTOMER_ID JOIN T11020 ON T14080.T_ENTRY_USER = T11020.T_USER_CODE WHERE T_SHOP_ID='{shopId}' AND T14080.T_FORM_CODE='T14020' AND T14080.T_VERIFY_FLG IS NULL");
            return sql;
        }
        public string SavePaymentVerifyData(List<T14024Data> t14024, string user)
        {
            var sms = "";
                    ...
                    foreach (var i in t14024)
                    {
                        ...
                        for (var k = 0; k < 4; k++)
                        {
                            ...
                            if (k == 0 && i.T_PAYMENT != 0)
                            {
                                head = "107";
                                purpous = "102";
                                narration = "Payment for Interior Shop";
                                debit = i.T_PAYMENT;
                                credit = 0;
                            }
